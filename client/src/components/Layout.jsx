...
export default function Layout() {
  ...
  return (
    <div className={`min-h-screen flex ${theme.primaryGradient}`}>
      {/* Sidebar for desktop */}
      <div className={`hidden lg:flex lg:flex-col lg:w-72 lg:fixed lg:inset-y-0 z-50 ${theme.sidebar}`}>
        <div className="flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4 pt-6">
          <div className="flex h-16 shrink-0 items-center justify-center mb-4">
            <span className={`text-2xl font-extrabold ${theme.gradientText} drop-shadow-lg tracking-tight`}>Admin Dashboard</span>
          </div>
                        ...
                        className={`
                          ...
                          ${
                            location.pathname === item.href
                              ? `${theme.primaryGradient} text-white shadow-lg scale-105`
          ...
          <div className="fixed inset-0 flex">
            ...
            >
              <Dialog.Panel className="relative mr-16 flex w-full max-w-xs flex-1">
                <div className={`flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4 pt-6 ${theme.sidebar}`}>
                  <div className="flex h-16 shrink-0 items-center justify-center mb-4">
                    <span className={`text-2xl font-extrabold ${theme.gradientText} drop-shadow-lg tracking-tight`}>Admin Dashboard</span>
                  </div>
                  <nav className="flex flex-1 flex-col">
      ...
      <div className="flex-1 flex flex-col lg:pl-72 min-h-screen">
        {/* Navbar */}
        <div className={`sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 ${theme.navbar} px-4 sm:px-6 lg:px-8`}>
          <button
            type="button"
          ...
          <div className="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
            ...
            <div className="flex items-center gap-x-4 lg:gap-x-6">
              ...
              <select
                ...
                className="rounded-lg px-2 py-1 bg-white/80 border border-gray-300 text-gray-700 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 mr-2"
                style={{ minWidth: 90 }}
                title="Change theme"
              >
